...
static constexpr const wchar_t* g_iniName = L"winMergeDark";
	...
	)
	{
		switch (uMsg)
		{
			...
			case WM_CTLCOLORSTATIC:
			{
				...
				{
					auto hWndChild = reinterpret_cast<HWND>(lParam);
					const int id = GetDlgCtrlID(hWndChild);
					if (id == _IDC_GNU_ASCII)
					{
						auto hdc = reinterpret_cast<HDC>(wParam);
						::SetTextColor(hdc, DarkMode::getDarkerTextColor());
						::SetBkMode(hdc, TRANSPARENT);
						return reinterpret_cast<LRESULT>(reinterpret_cast<HBRUSH>(::GetStockObject(NULL_BRUSH)));
					}
				}
				break;
			}
